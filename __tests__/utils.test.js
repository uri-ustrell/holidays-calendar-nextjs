...
describe("utils", () => {
	describe("getMonthDays", () => {
		...
		it("should return present month days if no date is passed", () => {
			const currentMonthDays = parseInt(
				dayjs()
					.endOf("month")
					.format("D"),
				10
			...
			expect(monthDays).toEqual(currentMonthDays);
		});
	});

	describe("getFirstDayPosition", () => {
		it("should return week position of the first day of month", () => {
			const position = getFirstDayPosition("2020-06-15T00:00:00+02:00");

			expect(position).toEqual(1);
		});

		it("shloud return position from current month if no date is passed", () => {
			const todayPosition = dayjs()
				.startOf("month")
				.get("d");
			const position = getFirstDayPosition();

			expect(position).toEqual(todayPosition);
		});
	});

	describe("formatDateMMMM", () => {
		it("should return date's month name", () => {
			const monthName = formatDateMMMM(
	...
	/* rest of utils to test ... */

	// this goes somwhere else
